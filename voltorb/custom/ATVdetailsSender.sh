...
# generic
    ...
    deviceName=$(su -c cat $mitm_conf | grep "origin" | awk -F">" '{ print $2 }' | awk -F"<" '{ print $1 }' || echo 'na')
    arch=$(uname -m)
...
# config
    ...
    rotomUrl=$(su -c cat $mitm_conf | grep "websocketurl" | awk -F">" '{ print $2 }' | awk -F"<" '{ print $1 }' || echo 'na')
